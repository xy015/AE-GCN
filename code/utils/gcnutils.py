...
# 计算损失函数
@tf_utils.function
def compute_loss(graph, logits, mask_index, vars):
    # 获取特定编号样本的预测结果
...
# 设置训练函数
@tf_utils.function
def train_step(model, graph, optimizer, train_index):
    # 计算损失函数
    with tf.GradientTape() as tape:
        logits = forward(model, graph, training=True)
        loss = compute_loss(graph, logits, train_index, tape.watched_variables())
    # 计算梯度
